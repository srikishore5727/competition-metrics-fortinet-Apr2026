...
// ─────────────────────────────────────────────────────────────
// WW Traffic Data  (Source: Semrush · Location: WW · Jul 2025 – Apr 2026)
// ─────────────────────────────────────────────────────────────
const ORGANIC_TRAFFIC_WW: Record<string, (number | null)[]> = {
  fortinet:     [4600000, 5200000, 4700000, 4500000, 4400000, 4900000, 4800000, 4900000, 5400000, 5100000],
  cisco:        [6100000, 6100000, 5800000, 19700000, 33500000, 26300000, 4400000, 5400000, 5500000, 5000000],
  hpe:          [1800000, 2000000, 1900000, 1700000, 1600000, 1700000, 1600000, 1500000, 1500000, 1400000],
  paloalto:     [700000,  900000,  1000000, 1300000, 1000000, 1000000, 900000,  1000000, 900000,  1000000],
  checkpoint:   [1000000, 1000000, 1000000, 1100000, 862000,  838000,  737000,  742000,  751000,  730000],
  crowdstrike:  [1065000, 2074000, 946000,  679000,  799000,  569000,  545000,  701000,  546000,  549000],
  catonetworks: [49000,   58000,   52000,   53000,   51000,   55000,   58000,   58000,   63000,   59000],
  sentinelone:  [346000,  348000,  363000,  405000,  511000,  383000,  413000,  509000,  444000,  487000],
  wiz:          [245000,  250000,  293000,  327000,  369000,  384000,  246000,  235000,  402000,  242000],
};

const BRANDED_TRAFFIC_WW: Record<string, (number | null)[]> = {
  fortinet:     [915790,  909300,  945910,  876570,  699660,  592310,  539740,  535500,  479330,  496100],
  cisco:        [2400000, 2400000, 2510000, 2170000, 1880000, 1680000, 1510000, 2290000, 2060000, 1600000],
  hpe:          [437920,  472980,  511020,  482410,  411390,  412380,  376850,  349900,  331060,  357250],
  paloalto:     [213470,  265170,  425440,  303500,  228210,  256460,  222660,  229450,  184550,  206410],
  checkpoint:   [191110,  209410,  203580,  192480,  179210,  159230,  157640,  139400,  126870,  124080],
  crowdstrike:  [647550,  1534900, 514460,  267720,  218810,  143380,  147050,  198360,  140990,  125630],
  catonetworks: [23650,   26240,   23690,   25650,   25090,   22370,   27170,   26710,   23710,   27750],
  sentinelone:  [166620,  156590,  157690,  155380,  156220,  149490,  142120,  151600,  102490,  131880],
  wiz:          [171510,  162800,  164290,  143800,  140090,  130490,  150230,  126920,  144160,  143600],
};

const NON_BRANDED_TRAFFIC_WW: Record<string, (number | null)[]> = {
  fortinet:     [3690000, 4260000, 3740000, 3620000, 3730000, 4300000, 4280000, 4330000, 4970000, 4620000],
  cisco:        [3660000, 3730000, 3310000, 17550000,31650000,24620000,2880000, 3130000, 3490000, 3380000],
  hpe:          [1390000, 1570000, 1380000, 1240000, 1160000, 1300000, 1190000, 1170000, 1190000, 1080000],
  paloalto:     [527750,  612870,  609700,  950640,  747050,  722390,  678780,  772520,  690110,  815410],
  checkpoint:   [825450,  827280,  824600,  870940,  682380,  678830,  579010,  602100,  623840,  605780],
  crowdstrike:  [417500,  539290,  431240,  411780,  579770,  425580,  397580,  502550,  405480,  422970],
  catonetworks: [25620,   32200,   28490,   27670,   26010,   32320,   30760,   30970,   39520,   30910],
  sentinelone:  [179060,  191390,  204820,  249250,  354290,  233820,  271010,  357130,  341200,  354770],
  wiz:          [73500,   87660,   129080,  183010,  228570,  253310,  96050,   108120,  257400,  98150],
};
      ...
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="success"
            title="Fortinet"
            content="From Jul 2025 to Apr 2026, WW traffic grew 4.6M → 5.1M (+10.9%), Wiz held 245K → 242K (−1.2%)."
          />
          <InsightCard
            icon={TrendingDown}
            type="error"
            title="Competitions"
            content="From Jul 2025 to Apr 2026, Cisco fell 6.1M → 5.0M (−18.0%), Crowdstrike 1.07M → 549K (−48.5%)."
          />
        </InsightsSection>
      );
      ...
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="error"
            title="Fortinet"
            content="From Jul 2025 to Apr 2026, WW branded fell 916K → 496K (−45.8%), Wiz 172K → 144K (−16.3%)."
          />
          <InsightCard
            icon={TrendingDown}
            type="error"
            title="Competitions"
            content="From Jul 2025 to Apr 2026, Cisco fell 2.40M → 1.60M (−33.3%), Palo Alto 213K → 206K (−3.3%)."
          />
        </InsightsSection>
      );
    } else {
      return (
        <InsightsSection>
          <InsightCard
            ...
            type="success"
            title="Fortinet"
            content="From Jul 2025 to Apr 2026, WW non-branded grew 3.69M → 4.62M (+25.2%), Wiz 74K → 98K (+33.5%)."
          />
          <InsightCard
            icon={TrendingDown}
            type="error"
            title="Competitions"
            content="From Jul 2025 to Apr 2026, Cisco fell 3.66M → 3.38M (−7.7%), Crowdstrike 418K → 423K (+1.3%)."
          />
        </InsightsSection>
      );
    }
